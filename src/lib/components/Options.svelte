<script lang="ts">
	...
	import InfinityIcon from '$lib/components/InfinityIcon.svelte';
	import MusicalNoteIcon from '$lib/components/MusicalNoteIcon.svelte';
	import RepeatSingleIcon from '$lib/components/RepeatSingleIcon.svelte';
	...
	<label class="label cursor-pointer gap-6">
		<span class="label-text text-base"
			><span class="flex items-center gap-2 font-bold"
				><RepeatSingleIcon class="h-5 w-5 flex-none" /> Speed controls just set their speed and never
				1x.</span
			><span class="mt-2 block text-sm"
				>By default, speed controls alternate between their speed (for example, 3x) and the default
				1x speed. Turn on this feature to have them just set their speed without going back to 1x.</span
			>
		</span>
		<input
...
<div class="form-control">
	<label class="label cursor-pointer gap-6">
		<span class="label-text text-base"
			><span class="flex items-center gap-2 font-bold"
				><InfinityIcon class="h-5 w-5 flex-none" /> Speed controls affect all media elements on a page.</span
			><span class="mt-2 block text-sm"
				>By default, speed controls only affect the first video in a page. Turn on this feature to
				have them affect all videos in a page. This lets you watch multiple videos on TikTok,
				Youtube Shorts or Twitter at faster speeds.</span
			></span
		>
...
<div class="form-control">
	<label class="label cursor-pointer gap-6">
		<span class="label-text text-base"
			><span class="flex items-center gap-2 font-bold"
				><MusicalNoteIcon class="h-5 w-5 flex-none" /> Speed controls also affect audio-only elements.</span
			><span class="mt-2 block text-sm"
